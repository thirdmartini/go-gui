...
func newConn(c net.Conn, width, height uint16) *Conn {
	...
	conn := &Conn{
		...
		fbupc:  make(chan FrameBufferUpdateRequest, 1),
...
func (c *Conn) dimensions() (w, h uint16) {
...
func (c *Conn) write(what string, v interface{}) {
	err := binary.Write(c.bw, binary.BigEndian, v)
	if err != nil {
		c.failf("writing into %T for %q: %v", v, what, err)
	}
	c.bw.Flush()
}
...
// any imgutil.Image concrete type and any client-requested pixel format.
...
func (c *Conn) handleUpdateRequest() {
	...
	//	fmt.Printf("FrameBuffer Request: %+v\n", req)
	c.fbupc <- req
}

// readString() returns formatted string field
